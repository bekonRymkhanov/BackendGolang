...
const (
	...
	createUsage       = `create [-ext E] [-dir D] [-seq] [-digits N] [-format] [-tz] NAME
	   Create a set of timestamped up/down migration titled NAME, in directory D with extension E.
...
`
	...
	upUsage   = `up [N]       Apply all or N up migration`
	downUsage = `down [N] [-all]    Apply all or N down migration
	Use -all to apply all down migration`
...
Options:
  -source          Location of the migration (driver://url)
  -path            Shorthand for -source=file://path
  -database        Run migration against this database (driver://url)
  -prefetch N      Number of migration to load in advance before executing (default 10)
...
Database drivers: `+strings.Join(database.List(), ", ")+"\n", createUsage, gotoUsage, upUsage, downUsage, dropUsage, forceUsage)
	...
	case "down":
		...
		applyAll := downFlagSet.Bool("all", false, "Apply all down migration")
		...
		if needsConfirm {
			log.Println("Are you sure you want to apply all down migration? [y/N]")
			var response string
			fmt.Scanln(&response)
			response = strings.ToLower(strings.TrimSpace(response))

			if response == "y" {
				log.Println("Applying all down migration")
			} else {
				log.fatal("Not applying all down migration")
			}
		}
